  ...
  useEffect(() => {
    const fetchDoubts = async () => {
      try {
        ...
        setDoubts(response.data?.doubts || []); // Set doubts or fallback to empty array
        setLoading(false);
      } catch (err) {
        setError(err.response?.data?.error || 'Failed to fetch doubts');
        setLoading(false);
        setDoubts([]); // Reset doubts to empty array on error
      }
    };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {doubts.map((doubt) => (
              <Link to={`/getdoubt/${doubt._id}`} key={doubt._id}> {/* Wrap card with Link */}
                <div
